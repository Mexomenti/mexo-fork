...
public class LocalPlayer : Entity
{
    ...
    public void UpdateWeapons()
    {
        ...
        if (punch) punch.GetComponentInChildren<SkinnedMeshRenderer>().material.mainTexture = ModAssets.HandTexture(Team);

        var right = cw?.transform.GetChild(0).Find("RightArm");
        if (right) right.GetComponentInChildren<SkinnedMeshRenderer>().material.mainTexture = ModAssets.HandTexture(Team, 1);

        var knuckle = fc.transform.Find("Arm Red(Clone)");
        if (knuckle) knuckle.GetComponentInChildren<SkinnedMeshRenderer>().material.mainTexture = ModAssets.HandTexture(Team, 2);
    }
